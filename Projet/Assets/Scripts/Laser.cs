using UnityEngine;
using System.Collections;
namespace AssemblyCSharp
{
	public class Laser : MonoBehaviour
	{
		public float laserWidth = 0.1f;
		public float noise = 1.0f;
		private float maxLength = 35.0f;
		public Color color;
		
		LineRenderer lineRenderer;
		int length;
		Vector3[] position;

		//Cache any transforms here
		Transform myTransform;
		Transform endEffectTransform;
		GameObject endEffect;
		Vector3 offset;

		void Start ()
		{
			myTransform = transform;
			offset = new Vector3(0,0,0);
			endEffect = Instantiate(Resources.Load("Prefab/LaserFX")) as GameObject;
			endEffect.transform.parent = transform;

			if(endEffect)
				endEffectTransform = endEffect.transform;
		}

		void Awake()
		{
			lineRenderer = gameObject.AddComponent<LineRenderer>() as LineRenderer;
			lineRenderer.SetWidth(laserWidth, laserWidth);
			lineRenderer.material = Resources.Load("Materials/LaserShot") as Material;
		}

		void Update ()
		{
			RenderLaser();
			RaycastHit hit = new RaycastHit();
			Ray ray = new Ray(transform.position, transform.forward);

			if(Physics.Raycast(ray, out hit))
			{
				if(hit.collider.gameObject.GetComponent<IAPon>() != null)
				{
					hit.collider.gameObject.GetComponent<IAPon>().Damage(2);
					//hit.collider.gameObject.rigidbody.AddForce(transform.forward * 100);
					//endEffectTransform = hit.collider.transform;
				}
			}
		}
		
		void RenderLaser()
		{
			//Shoot our laserbeam forwards!
			UpdateLength();
			
			lineRenderer.SetColors(color,color);
			//Move through the Array
			for(int i = 0; i<length; i++)
			{
				//Set the position here to the current location and project it in the forward direction of the object it is attached to
				offset.x =myTransform.position.x+i*myTransform.forward.x+Random.Range(-noise,noise);
				offset.y =myTransform.position.y+i*myTransform.forward.y+Random.Range(-noise,noise);
				offset.z =i*myTransform.forward.z+Random.Range(-noise,noise)+myTransform.position.z;
				position[i] = offset;
				position[0] = myTransform.position;
				lineRenderer.SetPosition(i, position[i]);	
			}
		}
		
		void UpdateLength()
		{
			//Raycast from the location of the cube forwards
			RaycastHit[] hit;
			hit = Physics.RaycastAll(myTransform.position, myTransform.forward, maxLength);
			int i = 0;

			while(i < hit.Length)
			{
				//Check to make sure we aren't hitting triggers but colliders
				if(!hit[i].collider.isTrigger)
				{
					length = (int)Mathf.Round(hit[i].distance) + 2;
					position = new Vector3[length];

					//Move our End Effect particle system to the hit point and start playing it
					if(endEffect)
					{
						endEffectTransform.position = hit[i].point;
						if(!endEffect.GetComponent<ParticleSystem>().isPlaying)
							endEffect.GetComponent<ParticleSystem>().Play();
					}
					lineRenderer.SetVertexCount(length);

					return;
				}

				i++;
			}
			//If we're not hitting anything, don't play the particle effects
			if(endEffect){
				if(endEffect.GetComponent<ParticleSystem>().isPlaying)
					endEffect.GetComponent<ParticleSystem>().Stop();
			}

			length = (int)maxLength;
			position = new Vector3[length];
			lineRenderer.SetVertexCount(length);
		}
	}
}
